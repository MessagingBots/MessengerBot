...
function receivedAuthentication(event) {
  ...
  (0, _sendUtils.sendTextMessage)(senderID, 'Authentication successful');
}
...
function receivedMessage(event) {
  ...
  if (isEcho) {
    ...
    (0, _sendUtils.sendTextMessage)(senderID, "Quick reply tapped");
    return;
  }
  ...
  if (messageText) {
    ...
    switch (messageText) {
      case 'courses':
        (0, _sendUtils.sendCourses)(senderID);
        break;

      case 'file':
        (0, _sendUtils.sendFileMessage)(senderID);
        break;

      case 'button':
        (0, _sendUtils.sendButtonMessage)(senderID);
        break;

      case 'quick reply':
        (0, _sendUtils.sendQuickReply)(senderID);
        break;

      case 'read receipt':
        (0, _sendUtils.sendReadReceipt)(senderID);
        break;

      case 'typing on':
        (0, _sendUtils.sendTypingOn)(senderID);
        break;

      case 'typing off':
        (0, _sendUtils.sendTypingOff)(senderID);
        break;

      case 'linking':
        (0, _sendUtils.sendAccountLinking)(senderID);
        break;

      default:
        (0, _sendUtils.sendTextMessage)(senderID, messageText);
    }
  } else if (messageAttachments) {
    (0, _sendUtils.sendTextMessage)(senderID, 'Message with attachment received');
  }
}
...
function receivedPostback(event) {
  ...
  (0, _sendUtils.sendTextMessage)(senderID, "Postback called");
}
...
exports.get = function (req, res) {
  if (req.query['hub.mode'] === 'subscribe' && req.query['hub.verify_token'] === _default2.default.fb.verifyToken) {
